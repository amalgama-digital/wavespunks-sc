...
describe('madpunks test', async function () {
    var arr = []
    var i = 0;
    ...
    it('Test positive: premint', async function () {
        for (let index = 1; index < arr.length; index++) {
            var mintTx = invokeScript({
                fee: 900000, 
            ...
            await broadcast(mintTx);
            await waitForTx(mintTx.id) 
        ...
        // await broadcast(mintTx);            
        expect(broadcast(mintTx)).to.be.rejectedWith("Mint hasn`t yet begun");
        await waitNBlocks(1) 
    })
    
    it('Test positive: mint', async function () {
    ...
    it('Test positive: all mint', async function () {
        for (let index = 0; index < 995; index++) {
            var mintTx = invokeScript({
                fee: 900000, 
                dApp: address(accounts.deploy),
                call: {
        ...
        expect(broadcast(mintTx)).to.be.rejectedWith("All punks are minted");
    })
